__all__ = ["EBNonlinearAssembler"]

import numpy as np
import scipy as sp
from ._eb_nl_elem import *

class EBNonlinearAssembler:
    def __init__(self, nxe:int, nxh:int, E:float, b:float, L:float, rho:float, 
        ...
        self.vpn = self.dof_per_node # shorthand version
        ...
        self.num_dof = self.vpn * self.num_nodes
        self.xscale = L / nxe
        self.dx = self.xscale
        self.bcs = [0, 1, self.vpn * (self.num_nodes-1) + 1] # u and w BCs
        self.conn = [[ielem, ielem+1] for ielem in range(self.num_elements)]

        self.u = np.zeros(self.num_dof)
    ...
    @property
    def dof_conn(self):
        return [[self.vpn * ix+_ for _ in range(2*self.vpn)] for ix in range(self.nxe)]

    def _compute_mat_vec(self, helem_vec):
        # copy states out
        E = self.E; b = self.b; rho = self.rho; nxe = self.nxe; L = self.L; rho_KS = self.rho_KS

        self.data = np.zeros((self.nnzb, self.vpn, self.vpn), dtype=helem_vec.dtype)
        ...
        for ielem in range(self.num_elements): 
            ...
            elem_disp = self.u[local_conn]

            EI = E * b * helem_vec[ielem]**3 / 12.0
            EA = E * b * helem_vec[ielem]

            # compute kelem here
            Kelem_nl = get_nl_kelem(self.xscale / 2.0, EI, EA, elem_disp)
            felem_nom = get_felem(self.xscale / 2.0)

            if self._dense:
                np.add.at(Kmat, (local_conn[:,None], local_conn[None,:]), Kelem_nl)
            else: # sparse
                ...
                for row_node in [ielem, ielem+1]:
                    ...
                    for p in range(start, end):
                        ...
                        if col_node in [ielem, ielem+1]:
                            ...
                            Kelem_loc = Kelem_nl[self.vpn*inode:(self.vpn*inode+self.vpn)][:,self.vpn*jnode:(self.vpn*jnode+self.vpn)]
        ...
        # now apply simply supported  (with membrane now also)
        bcs = [0, 1, self.vpn * (self.num_nodes-1) + 1]
        ...
        if self._dense:
            ...
            # w BC right side
            bc = self.vpn * (self.num_nodes-1) + 1
            self.data[self.nnzb-2:,1,:] = 0.0
            self.data[self.nnzb-3,:,1] = 0.0
            self.data[self.nnzb-1,:,1] = 0.0
            self.data[self.nnzb-1,1,1] = 1.0
        ...
        if not self._dense:
            Kmat = sp.sparse.bsr_matrix(
                ...
                shape=(self.vpn*self.num_nodes, self.vpn*self.num_nodes))
    ...
    def solve_local(self, hred, lam:float=1.0):
        # now solve the linear system
        helem_vec = self.get_helem_vec(hred)
        self._compute_mat_vec(helem_vec)

        # compute residual
        self.res = lam * self.force - self.Kmat.dot(self.u)
        res_nrm = np.linalg.norm(self.res)

        if self._dense:
            self.du = np.linalg.solve(self.Kmat, self.res)
        else:
            # print(f"{self.Kmat.toarray()=}")
            self.du = sp.sparse.linalg.spsolve(self.Kmat, self.res)

        self.u += self.du
        return self.u.copy(), res_nrm

    def solve_load_newton(self, hred, lams:list, rtol=1e-6):
        # naive or basic nonlinear newton solver
        solns = []

        nload_steps = len(lams)
        for ilam,lam in enumerate(lams):

            print(f"Load step {ilam+1}/{nload_steps} : {lam=:.2e}")

            for inewton in range(20):

                soln, res_nrm = self.solve_local(hred, lam)
                print(f"\t{inewton=} : {res_nrm=:.2e}")
                if inewton == 0:
                    init_nrm = res_nrm
                elif res_nrm < init_nrm * rtol:
                    break

            # self.plot_disp(idof=0)
            # self.plot_disp(idof=1)
            solns += [soln.copy()]

        return solns, res_nrm

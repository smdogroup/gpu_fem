...
void multigrid_plate_solve(int nxe, double SR, int nsmooth, int ninnercyc, T omega, std::string cycle_type) {
...
void gatekeeper_method(bool is_multigrid, int nxe, double SR, int nsmooth, int ninnercyc, T omega, std::string cycle_type) {
    if (is_multigrid) {
        multigrid_plate_solve<T, Assembler>(nxe, SR, nsmooth, ninnercyc, omega, cycle_type);
    } else {

...
void multigrid_plate_solve(int nxe, double SR, int n_iters, double omega, std::string smooth_name) {
    ...
    // printf("building fine grid assembler\n");
    auto fine_assembler = createPlateAssembler<Assembler>(nxe, nye, Lx, Ly, E, nu, thick, rho, ys, nxe_per_comp, nye_per_comp);
    double Q = 1.0; // load magnitude
    // printf("getting fine grid loads\n");
    T *my_loads = getPlateLoads<T, Physics>(nxe, nye, Lx, Ly, Q);
    // printf("making fine grid object\n");
    GRID *fine_grid = GRID::buildFromAssembler(fine_assembler, my_loads);
    ...
    plotSolution<T, Assembler, GRID>(coarse_grid, coarse_grid->d_soln, "out/1_plate_coarse_soln.vtk");
    ...
    plotSolution<T, Assembler, GRID>(fine_grid, fine_grid->d_defect, "out/2_plate_fine_defect.vtk");
    ...
    outputFile << "out/3_plate_" << smooth_name << "_omega_" << std::to_string(omega) << ".vtk";

...
template <typename T>
__global__ void k_copyMatValuesToBatched(const int n_batch_vals, const int block_dim, const int size, 
    const int *d_blockMap, const T *vals, T **array_vals) {
    // copies matrix values from original BSR mat to batched storage
    // in each Schwarz coupling group
    
    // compute the appropriate indices in Aarray in order
    int tid = threadIdx.x + blockDim.x * blockIdx.x;
    if (tid >= n_batch_vals) return;
    int block_dim2 = block_dim * block_dim;
    int batch_block_ind = n_batch_vals / block_dim2;
    int size2 = size * size;
    int size4 = size2 * size2;
    int batch_ind = batch_block_ind / size4;
    int inner_block_ind = batch_block_ind % size4; 

    // get the block ind in the kmat
    int kmat_block_ind = d_blockMap[batch_block_ind];

    // on each thread perform the copy operation
    int inner_ind = n_batch_vals % block_dim2;
    T val = vals[block_dim2 * kmat_block_ind + inner_ind];
    T *batch_ptr = array_vals[batch_ind];
    batch_ptr[block_dim2 * inner_block_ind + inner_ind] = val;
}
...
__global__ void k_copyRHSIntoBatched(const int n_batch_vals, const int block_dim, const int size,
    ...
    if (tid >= n_batch_vals) return;
    int block_dim2 = block_dim * block_dim;
    int batch_block_ind = n_batch_vals / block_dim;
    int size2 = size * size;
    ...
    // on each thread perform the copy operation
    int inner_ind = n_batch_vals % block_dim;
...
__global__ void k_copyBatchedIntoSoln(const int n_batch_vals, const int block_dim, const int size,
    const int *d_blockMap, const T *array_soln, T *soln) {
    // copies original rhs vector into batched storage for each Schwarz coupling group

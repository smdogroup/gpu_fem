...
class BeamFem:
    ...
    def _compute_mat_vec(self, helem_vec):
        ...
        qvec = [self.qmag * np.sin(np.exp(3.0) * np.pi * xval / L) * np.sin(np.pi * xval / L) for xval in xvec]
    ...
    def solve_forward(self, hred):
        ...
        if self._dense:
            ...
            self.u = sp.sparse.linalg.spsolve(self.Kmat_csr, self.force)

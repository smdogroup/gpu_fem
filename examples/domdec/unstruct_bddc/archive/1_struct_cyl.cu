...
int main(int argc, char **argv) {
    ...
    // using Splitter = UnstructMacroElementSplitter;
    // using Splitter = UnstructCompactPatchSplitter;
    // using Splitter = GlobalLocalCornerOptSplitter;
    // using Splitter = GlobalLocalCornerOptSplitterV2;
    // using Splitter = GlobalLocalCornerOptSplitterV3;
    using Splitter = MetisCornerOptSplitter;
    ...
    int nxe = 100, nxe_subdomain_size = 4;  // subdomain = 5 works too sometimes
    // int nxe = 256, nxe_subdomain_size = 4; // 8 subdomains slightly faster (cause shrinks coarse
    // problem) for local + HPC int nxe = 512, nxe_subdomain_size = 4; // 8 subdomains slightly
    // faster (cause shrinks coarse problem) for local + HPC
    T thick = 1e-3;
    T mag = 1.0e2;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (strcmp(arg, "--nxe") == 0) {
            ...
            std::cerr << "Usage: " << argv[0]
                      << " [direct/krylov] [--nxe value] [--SR value] [--nsmooth int]" << std::endl;
            return 1;
        }
    }
    ...
    auto split = new IEVSplit(num_elements, num_nodes, nodes_per_elem, h_elem_conn, target_sd_size);

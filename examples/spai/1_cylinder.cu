...
template <typename T, class Assembler>
void gatekeeper_method(std::string solver_type, int nxe, double SR, int optim, T omega, int fill, T load_mag = 5.0e7) {
    if (solver_type == "direct") {
        solve_direct<T, Assembler>(nxe, SR, load_mag);
    } else if (solver_type == "spai") {
        spai_solve<T, Assembler>(nxe, SR, fill, optim, load_mag);
    }
}
...
int main(int argc, char **argv) {
    ...
    gatekeeper_method<T, Assembler>(solver_type, nxe, SR, optim, omega, fill, pressure);

...
parser.add_argument("--cf_smooth", type=int, default=1, help="coarse-fine smooth boolean")
...
P, R = gmg_plate_coarse_fine_matrices(nxe_fine, apply_bcs=False)
...
K_fine, F_fine = apply_bcs(nxe_fine, _K, _F) # this version just zeros out in place

true_soln = np.linalg.solve(K_fine, F_fine)
...
for i_vcycle in range(100):
    ...
    # 1 - fine pre-smooth
    du_f_1 = block_gauss_seidel(K_f_csr, defect_f_0.copy(), x0=np.zeros(ndof_fine), num_iter=1)
    soln_1 = soln_0 + du_f_1
    defect_f_1 = defect_f_0 - K_f_csr.dot(du_f_1)

    plot_compare(defect_f_0, "defect0", defect_f_1, "defect1")
    ...
    defect_c_0 = np.dot(P.T, defect_f_1)

    # check_defect(defect_f_1, soln_1)
    plot_compare(defect_f_1, "fine-defect", defect_c_0, "coarse-defect")

    # 3 - solve on coarse level
    du_c_1 = spsolve(K_c_csr, defect_c_0)

    # 4 - coarse to fine (prolongate)
    du_f_2 = np.dot(P, du_c_1)

    # coarse-fine smooth
    if args.cf_smooth:
        df_f_2 = K_f_csr.dot(du_f_2)
        du_cf_smooth = block_gauss_seidel(K_f_csr, -df_f_2.copy(), x0=np.zeros(ndof_fine), num_iter=1)
        du_f_2 += du_cf_smooth
    ...
    plot_compare(du_c_1, "coarse-disps", du_f_2, "fine-disps")

    # plot defect before and after coarse-fine
    plot_compare(defect_f_1, "defect-pre-cf", defect_f_2, "defect-post-cf")

    # 5 - fine post-smooth
    du_f_3 = block_gauss_seidel(K_f_csr, defect_f_2.copy(), x0=np.zeros(ndof_fine), num_iter=1)
    soln_3 = soln_2 + du_f_3    
    defect_f_3 = defect_f_2 - K_f_csr.dot(du_f_3)

    plot_compare(defect_f_2, "defect_f_2", defect_f_3, "defect_f_3")

...
template <typename T, class Assembler, class Prolongation>
void multigrid_plate_solve(int nxe, double SR, T omega, int ORDER, int nsmooth, int nsmooth_matrix, int ninnercyc, std::string cycle_type, std::string prolong_type) {
    ...
    for (int c_nxe = nxe; c_nxe >= nxe_min; c_nxe /= 2) {
        ...
        // int ORDER = 4; // for chebyshev
        // int ORDER = 1;
        auto smoother = new Smoother(cublasHandle, cusparseHandle, assembler, kmat, omega, ORDER);
        smoother->setup_cg_lanczos(loads);

        Prolongation *prolongation = nullptr;
        if constexpr (Prolongation::smoothed) {
            int ELEM_MAX = 10;
            prolongation = new Prolongation(cusparseHandle, assembler, ELEM_MAX);
        } else {
            prolongation = new Prolongation(assembler);
        }
        // int smooth_matrix_iters = 1; // with order 4
        // int smooth_matrix_iters = 2; // with order 4
        auto grid = GRID(assembler, prolongation, smoother, kmat, loads, cublasHandle, cusparseHandle, omegaLS_min, omegaLS_max, nsmooth_matrix);
    ...
    bool double_smooth = false; // sometimes ends up being slower..
...
template <typename T, class Assembler, class Prolongation>
void gatekeeper_method(bool is_multigrid, int nxe, double SR, T omega, int ORDER, int nsmooth, int nsmooth_matrix, int ninnercyc, std::string cycle_type, std::string prolong_type) {
    if (is_multigrid) {
        multigrid_plate_solve<T, Assembler, Prolongation>(nxe, SR, omega, ORDER, nsmooth, nsmooth_matrix, ninnercyc, cycle_type, prolong_type);
    } else {
...
int main(int argc, char **argv) {
    ...
    int ORDER = 4; // default chebyshev
    int nsmooth_mat = 1; // 1 is actually better usually, huh, can half the number of smoothing steps
    // TODO : if need orthog projector or how to get better smoothing out of it..
    double omega = 0.8;

    std::string cycle_type = "K"; // "V", "F", "W", "K"
    // std::string elem_type = "CFI4"; // 'MITC4', 'CFI4', 'CFI9'
    std::string elem_type = "MITC4";
    std::string prolong_type = "smooth"; // "smooth", "unsmooth" or else is unsmooth
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        //         return 1;
        //     }
        } else if (strcmp(arg, "--prolong") == 0) {
            if (i + 1 < argc) {
                prolong_type = argv[++i];
            } else {
                std::cerr << "Missing value for --prolong\n";
                return 1;
            }
    ...
    if (prolong_type == "smooth") {
        const bool KMAT_FILLIN = true;
        using Prolongation = UnstructuredSmoothProlongation<Assembler, Basis, KMAT_FILLIN>;
        gatekeeper_method<T, Assembler, Prolongation>(is_multigrid, nxe, SR, omega, ORDER, nsmooth, nsmooth_mat, ninnercyc, cycle_type, prolong_type);
    } else {

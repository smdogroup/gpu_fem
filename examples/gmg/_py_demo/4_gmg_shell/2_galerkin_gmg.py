...
I_cf, I_fc = coarse_fine_method(nxe, sort_bk_map_list[0], sort_bk_map_list[1], 
                                      bcs_list=[bcs_list[0], bcs_list[1]] if remove_bcs else None)

P = I_cf.copy()

# row-normalize it.. (so outputs always 1)
for row in range(P.shape[0]):
    P[row] /= np.sum(P[row]) + 1e-16 # since some bcs zeroed out..

# remove coarse bcs from this..
P[:,bcs_list[1]] = 0.0
P[bcs_list[0], :] = 0.0 # and fine bcs..

# plt.imshow(P)
# plt.show()

res_c = np.dot(P.T, res)
res2_c = np.dot(P.T, res2)
res_f = np.dot(P, res_c)
res2_f = np.dot(P, res2_c)
...
# instead use galerkin coarse matrix (not same as coarse discretized stiffness matrix due to MITC
_lhs_1_gal = P.T @ lhs_0.toarray() @ P

# modify bcs here..
for bc in bcs_list[1]:
    _lhs_1_gal[bc,bc] = 1.0

lhs_1_gal = sp.sparse.csr_matrix(_lhs_1_gal)

# apply bcs to lhs_1 gal


# r_c = np.random.rand(P.shape[-1])
# u_c_test = spsolve(lhs_1_gal, r_c)
# print(f"{np.linalg.norm(u_c_test)=:.2e}")
# exit()

for v_cycle in range(args.n_vcyc):
    ...
    defect_1_0 = np.dot(P.T, defect_0_1)

    # solve at level 1 (NOTE : we're just doing 2 grids for rn)
    # lhs_1 = tacs_csr_mat_list[1]
    dx_1 = spsolve(lhs_1_gal, defect_1_0)

    # coarse-fine the disps
    _dx_cf_0 = np.dot(P, dx_1)

    if args.plot:
        print(f"{v_cycle=} : 2.0 - coarse solve..")
        plot_vec_compare_all(nxe_list[1], defect_1_0, dx_1, sort_fw_map=sort_fw_map_list[1], 
                                filename=None)

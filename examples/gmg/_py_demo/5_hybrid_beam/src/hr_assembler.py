__all__ = ["HRBeamAssembler"]

import numpy as np
import scipy as sp
from ._hr_elem import *

class HRBeamAssembler:
    # hellinger-reissner timoshenko beam element based on https://onlinelibrary.wiley.com/doi/epdf/10.1002/nme.5766?saml_referrer
    def __init__(self, nxe:int, nxh:int, E:float, b:float, L:float, rho:float, 
        ...
        self.xscale = L / nxe
        self.dx = self.xscale
        self.bcs = [0, 2, 3 * (self.num_nodes-1), 3 * (self.num_nodes - 1) + 2]
    ...
    def _compute_mat_vec(self, helem_vec):
        ...
        if self._dense:
            ...
            self.data[self.nnzb-2:,2,:] = 0.0
            self.data[self.nnzb-3,:,2] = 0.0
            self.data[self.nnzb-1,:,2] = 0.0
            self.data[self.nnzb-1,2,2] = 1.0
    ...
    def prolongate(self, coarse_disp):
        ...
        for ielem_c in range(nelems_coarse):
            ...
            for i, inode_f in enumerate(range(2 * ielem_c, 2 * ielem_c + 3)):
                ...
                w, th, ws = interp_lagrange(xi, coarse_elem_disps)

                fine_disp[3 * inode_f] += w
                fine_disp[3 * inode_f + 1] += th
                fine_disp[3 * inode_f + 2] += ws
        ...
        # print(f"{coarse_disp.shape=} => {fine_disp.shape=}")
    ...
    def restrict_defect(self, fine_defect):
        ...
        for ielem_c in range(nelems_coarse):
            ...
            for i, inode_f in enumerate(range(2 * ielem_c, 2 * ielem_c + 3)):
                ...
                # print(f"{fine_defect[:5]=} {fine_weights[:5]=}")
                # print(f"{nodal_in=} {xi=}")
        ...
        # print(f"{fine_defect.shape=} {coarse_defect.shape=}")

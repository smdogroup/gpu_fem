...
#include "domdec/subdomains/unstruct/metis_wing.h"
...
int run_solver(MPI_Comm &comm, int level, int target_sd_size, double thick, double mag) {
    ...
    printToVTK_v2<T, Assembler>(
        *assembler, h_soln, "./out/2_unstruct_cyl.vtk");
...
int main(int argc, char **argv) {
    ...
    double mag = 1e2;
    int level = 2;
    std::string splitter = "metis-wing-opt";
...
#define RUN_SPLITTER(name, type)                    \
    if (splitter == name)                           \
        return run_solver<type>(                    \
            comm, level, subdomain, thick, mag)

    RUN_SPLITTER("metis", MetisCornerOptSplitter<false>);
    RUN_SPLITTER("metis-opt", MetisCornerOptSplitter<true>);
    RUN_SPLITTER("metis-wing", MetisWingOptSplitter<false>);
    RUN_SPLITTER("metis-wing-opt", MetisWingOptSplitter<true>);

...
class LinearMITCWingSolver {
   public:
    ...
    static const bool is_nonlinear = true;  // cause linear wing
    ...
    // using Assembler = FullyIntegratedShellAssembler<T, Director, Basis, Physics, DeviceVec,
    // BsrMat>;
    ...
    LinearMITCWingSolver(double rhoKS = 100.0, double safety_factor = 1.5, double force = 30e3,
        ...
        for (int i = level; i >= 0; i--) {
            ...
            std::string fname =
                "../../multigrid/3_aob_wing/meshes/aob_wing_L" + std::to_string(i) + ".bdf";

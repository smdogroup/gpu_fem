...
for i in range(4):
    ...
    plt.plot(toverR, _arr, "o-", linewidth=3, color=colors[i], label=_name)
...
for i in range(4):
    ...
    #     _arr[_arr > 2.0] = np.nan
    if i < 3:
        plt.plot(NDOF, _arr, "o-", linewidth=3, color=colors[i], label=_name)
    else:  # wing
        plt.plot(arr2[:,2], _arr, "o-", linewidth=3, color=colors[i], label=_name)

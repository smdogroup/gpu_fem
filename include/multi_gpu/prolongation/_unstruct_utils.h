...
template <typename T, class Basis, bool is_bsr, bool include_restrict = true>
void init_unstructured_grid_maps(const int block_dim, T *h_xpts_fine, T *h_xpts_coarse,
                                 const int nnodes_fine, const int nnodes_coarse, int *h_fine_conn,
                                 int *h_coarse_conn, const int num_fine_elems,
                                 const int num_coarse_elems, int *h_fine_elem_comps,
                                 int *h_coarse_elem_comps, int *&d_n2e_ptr, int *&d_n2e_elems,
                                 T *&d_n2e_xis, BsrData &P_bsr_data, BsrData &PT_bsr_data,
                                 DeviceVec<T> &d_P_vals, DeviceVec<T> &d_PT_vals,
    ...
    // no permutations needed for this version
    int *h_iperm = new int[nnodes_fine];
    int *h_perm = new int[nnodes_fine];
    int *h_coarse_iperm = new int[nnodes_coarse];
    int *h_coarse_perm = new int[nnodes_coarse];
    memset(h_perm, 0, nnodes_fine * sizeof(int));
    memset(h_iperm, 0, nnodes_fine * sizeof(int));
    for (int node = 0; node < nnodes_fine; node++) {
        h_perm[node] = node, h_iperm[node] = node;
    }
    for (int node = 0; node < nnodes_coarse; node++) {
        h_coarse_iperm[node] = node, h_coarse_perm[node] = node;
    }
    int *d_perm = HostVec<int>(nnodes_fine, h_perm).createDeviceVec().getPtr();
    int *d_iperm = HostVec<int>(nnodes_fine, h_iperm).createDeviceVec().getPtr();
    int *d_coarse_iperm = HostVec<int>(nnodes_coarse, h_coarse_iperm).createDeviceVec().getPtr();
    int *d_coarse_perm = HostVec<int>(nnodes_coarse, h_coarse_perm).createDeviceVec().getPtr();
    ...
    int P_block_dim = block_dim;  // if !is_bsr then it does same prolong on each dof_per_node
    ...
    d_P_vals = DeviceVec<T>(block_dim2 * P_nnzb);
    P_bsr_data =
    ...
    if constexpr (include_restrict) {
        ...
        d_PT_vals = DeviceVec<T>(block_dim2 * PT_nnzb);
        PT_bsr_data = BsrData(nnodes_coarse, P_block_dim, PT_nnzb, d_PT_rowp, d_PT_cols,

class UnstructEdgeFillSplitterV2 {
   public:
    ...
    UnstructEdgeFillSplitterV2(int num_elements_, int num_nodes_, int nodes_per_elem_,
    ...
    void split(const std::vector<int> &ne_ptr, const std::vector<int> &ne_cols,
        ...
        assign_initial_subdomains_edgefill_v2(ne_ptr, ne_cols, elem_adj);
   ...
   private:
    void die(const char *msg) const {
        printf("ERROR[UnstructEdgeFillSplitterV2]: %s\n", msg);
        std::exit(1);
    }
    ...
    int checked_node(int ielem, int lnode) const {
        ...
        if (gnode < 0 || gnode >= num_nodes) {
            printf("ERROR[UnstructEdgeFillSplitterV2]: bad node %d elem %d lnode %d\n", gnode,
                   ielem, lnode);
            std::exit(1);
        }
        return gnode;
        ...
        for (int ielem = 0; ielem < num_elements; ielem++) {
            ...
            for (int jp = ee_ptr[ielem]; jp < ee_ptr[ielem + 1]; jp++) {
                ...
                if (jelem < 0 || jelem >= num_elements) {
                    printf("ERROR[UnstructEdgeFillSplitterV2]: bad ee elem=%d nbr=%d\n", ielem,
                           jelem);
                    std::exit(1);
                }
    ...
    bool edge_node_is_near_corner_node(int edge_node, int isd, const std::vector<int> &ne_ptr,
                                       const std::vector<int> &ne_cols) const {
        // edge_node has already been verified to have 2 elems in this subdomain.
        // It is "near a corner" if one of those incident sd elems also touches
        // a node with exactly 1 elem in this subdomain.
        for (int jp = ne_ptr[edge_node]; jp < ne_ptr[edge_node + 1]; jp++) {
            int e = ne_cols[jp];
            if (elem_sd_ind[e] != isd) continue;

            for (int lnode = 0; lnode < nodes_per_elem; lnode++) {
                int gnode = checked_node(e, lnode);
                if (gnode == edge_node) continue;
    ...
    void collect_edge_expansion(int gnode, int isd, const std::vector<int> &ne_ptr,
        ...
        for (int jp = ne_ptr[gnode]; jp < ne_ptr[gnode + 1]; jp++) {
            int e = ne_cols[jp];
            if (elem_sd_ind[e] < 0) add_elems.push_back(e);
        }
    ...
    void assign_initial_subdomains_edgefill_v2(const std::vector<int> &ne_ptr,
                                               const std::vector<int> &ne_cols,
            ...
            while (sd_size < target_sd_size) {
                ...
                for (int e : sd_elems) {
                    for (int lnode = 0; lnode < nodes_per_elem; lnode++) {
                        ...
                        // V2 edge node definition.
                        if (n_sd_at_node != 2) continue;

                        // V2 restriction: only expand edge nodes adjacent to a corner node.
                        if (!edge_node_is_near_corner_node(gnode, isd, ne_ptr, ne_cols)) continue;

                        std::vector<int> add_elems;
                        collect_edge_expansion(gnode, isd, ne_ptr, ne_cols, add_elems);

                        // Boundary edge nodes with no outside elements are ignored.
                        if (add_elems.empty()) continue;

                        // Keep subdomain size <= target.
    ...
    void compact_subdomain_ids() {
        ...
        for (int e = 0; e < num_elements; e++) {
            ...
            if (old_isd < 0 || old_isd >= num_subdomains || sd_imap[old_isd] < 0) {
                printf("ERROR[UnstructEdgeFillSplitterV2]: bad compact elem=%d old_isd=%d\n", e,
                       old_isd);
                std::exit(1);
            }

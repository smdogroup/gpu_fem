...
class GlobalLocalCornerOptSplitter {
   public:
    ...
    GlobalLocalCornerOptSplitter(int num_elements_, int num_nodes_, int nodes_per_elem_,
   ...
   private:
    ...
    int choose_best_initial_elem(const std::vector<int> &frontier, int isd,
        ...
        // Allow neutral early growth, but prefer non-worsening.
    ...
    void local_corner_optimization(const std::vector<int> &ne_ptr, const std::vector<int> &ne_cols,
        ...
        for (int pass = 0; pass < max_local_passes; pass++) {
            ...
            printf("GlobalLocalCornerOptSplitter pass %d: corner violations %d -> %d\n", pass,
                   old_total, new_total);

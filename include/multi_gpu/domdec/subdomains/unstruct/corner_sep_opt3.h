...
class GlobalLocalCornerOptSplitterV3 {
    // now in regional rebuilds, it allows subdomains of different sizes, since when nxe and
    // nxe_subdomain_size don't divide each other you may need larger / smaller subdomains to avoid
    // corner violations.. i.e. 100x100 mesh with 3x3 subdomains would need some 4x4 or 2x2
    // subdomains to preserve regularity
   ...
   public:
    ...
    // Regional rebuild flexibility:
    // ideal_count = round(region_elems / target_sd_size)
    // candidate rebuilt counts can go down to about 60% of ideal_count.
    double regional_min_count_fraction = 0.60;
    double regional_max_count_fraction = 1.40;

    // Allows rebuilt regional pieces to be larger than normal local moves.
    double regional_max_size_fraction = 1.75;

    GlobalLocalCornerOptSplitterV3(int num_elements_, int num_nodes_, int nodes_per_elem_,
    ...
    void split(const std::vector<int> &ne_ptr, const std::vector<int> &ne_cols,
        ...
        if (remaining > 0) {
            printf("WARNING[GlobalLocalCornerOptSplitterV2]: %d corner violations remain\n",
                   remaining);
        }
    }
   ...
   private:
    void die(const char *msg) const {
        printf("ERROR[GlobalLocalCornerOptSplitterV2]: %s\n", msg);
        std::exit(1);
    }
    ...
    int checked_node(int e, int lnode) const {
        ...
        if (gnode < 0 || gnode >= num_nodes) {
            printf("ERROR[GlobalLocalCornerOptSplitterV2]: bad node %d elem %d lnode %d\n", gnode,
                   e, lnode);
            std::exit(1);
        }
        return gnode;
    ...
    void local_corner_optimization(const std::vector<int> &ne_ptr, const std::vector<int> &ne_cols,
        ...
        for (int pass = 0; pass < max_local_passes; pass++) {
            ...
            printf("GlobalLocalCornerOptSplitterV2 local pass %d: corner violations %d -> %d\n",
    ...
    void build_balanced_region_targets(int nregion, int nsd_region,
                                       std::vector<int> &targets) const {
        targets.clear();
        if (nregion <= 0 || nsd_region <= 0) return;

        if (nsd_region > nregion) nsd_region = nregion;

        int base = nregion / nsd_region;
        int rem = nregion % nsd_region;

        targets.reserve(nsd_region);
        for (int i = 0; i < nsd_region; i++) {
            targets.push_back(base + (i < rem ? 1 : 0));
        }

        // Larger targets first usually reduce tiny tail fragments.
        std::sort(targets.begin(), targets.end(), std::greater<int>());
    }

    void build_regional_count_candidates(int nregion, std::vector<int> &candidates) const {
        candidates.clear();
        if (nregion <= 0) return;

        int ideal = std::max(1, (nregion + target_sd_size / 2) / target_sd_size);

        int relaxed_max_size =
            std::max(max_sd_size, (int)(regional_max_size_fraction * (double)target_sd_size));

        int hard_min_count = std::max(1, iceil_div(nregion, relaxed_max_size));
        int hard_max_count = nregion;

        int frac_min_count =
            std::max(1, (int)(regional_min_count_fraction * (double)ideal + 0.999999));
        int frac_max_count =
            std::max(1, (int)(regional_max_count_fraction * (double)ideal + 0.999999));

        int min_count = std::max(hard_min_count, frac_min_count);
        int max_count = std::min(hard_max_count, frac_max_count);

        if (min_count > max_count) {
            min_count = hard_min_count;
            max_count = std::min(hard_max_count, std::max(hard_min_count, ideal));
        }

        // Try ideal first, then fewer domains, then more domains.
        auto add_candidate = [&](int n) {
            if (n < min_count || n > max_count) return;
        ...
        for (int d = 1; d <= std::max(ideal - min_count, max_count - ideal); d++) {
            add_candidate(ideal - d);
            add_candidate(ideal + d);
        }
    }

    bool rebuild_region_with_count(const std::vector<int> &region_elems, int nsd_region,
                                   const std::vector<int> &ne_ptr, const std::vector<int> &ne_cols,
                                   const std::vector<std::vector<int>> &elem_adj) {
        ...
        build_balanced_region_targets((int)region_elems.size(), nsd_region, targets);
        ...
        for (int target : targets) {
            ...
            while (sd_size < target && num_left > 0) {
        ...
        // Fallback: if frontiers stalled, grow extra connected pieces for remaining elements.
        while (num_left > 0) {
            int seed = choose_region_seed(region_elems, region_flag);
            if (seed < 0) break;
            ...
            int isd = num_subdomains++;
            int sd_size = 0;
            int target = std::min(target_sd_size, num_left);
        ...
        for (int e : region_elems) {
            if (elem_sd_ind[e] < 0) return false;
        }
    ...
    bool try_rebuild_region(const std::vector<int> &region_sds, const std::vector<int> &ne_ptr,
        ...
        if ((int)region_elems.size() < min_sd_size) return false;
        if ((int)region_elems.size() > 12 * target_sd_size) return false;

        std::vector<int> candidates;
        build_regional_count_candidates((int)region_elems.size(), candidates);

        int best_global = old_global;
        int best_num_subdomains = old_num_subdomains;
        std::vector<int> best_assign;
        int best_nsd_region = -1;

        for (int nsd_region : candidates) {
            elem_sd_ind = old_assign;
            num_subdomains = old_num_subdomains;

            bool ok =
                rebuild_region_with_count(region_elems, nsd_region, ne_ptr, ne_cols, elem_adj);
            if (!ok) continue;
            ...
            if (new_global < best_global) {
                ...
                best_assign = elem_sd_ind;
                best_num_subdomains = num_subdomains;
                best_nsd_region = nsd_region;

                if (best_global == 0) break;
            }
        }

        if (best_global < old_global) {
            elem_sd_ind = best_assign;
            num_subdomains = best_num_subdomains;

            printf(
                "GlobalLocalCornerOptSplitterV2 regional rebuild accepted: %d -> %d, "
                "region_elems=%d old_region_sds=%d new_region_sds=%d\n",
                old_global, best_global, (int)region_elems.size(), (int)region_sds.size(),
                best_nsd_region);
    ...
    void regional_rebuild_optimization(const std::vector<int> &ne_ptr,
        ...
        for (int pass = 0; pass < max_regional_passes; pass++) {
            ...
            printf("GlobalLocalCornerOptSplitterV2 regional pass %d: corner violations %d -> %d\n",
                   pass, old_total, new_total);
    ...
    void compact_subdomain_ids() {
        ...
        for (int e = 0; e < num_elements; e++) {
            ...
            if (old_isd < 0 || old_isd > max_old_sd || sd_imap[old_isd] < 0) {
                printf("ERROR[GlobalLocalCornerOptSplitterV2]: bad compact elem=%d old_isd=%d\n", e,
                       old_isd);
                std::exit(1);
            }

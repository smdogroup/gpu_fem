   ...
   public:
    ...
    void setElementComponents(const int *h_elem_comp, int **h_loc_elem_comp,
        ...
        for (int g = 0; g < ngpus; g++) {
            ...
            CHECK_CUDA(cudaSetDevice(debug ? 0 : g));
            CHECK_CUDA(cudaMalloc((void **)&d_loc_elem_comp[g], nloc * sizeof(int)));
            CHECK_CUDA(cudaMemcpy(d_loc_elem_comp[g], h_loc_elem_comp[g], nloc * sizeof(int),
        ...
        for (int g = 0; g < ngpus; g++) {
            ...
            if (print_maps) {
                printf("GPU[%d]: local_nelems=%d owned_nnodes=%d local_nnodes=%d\n", g,
                       local_nelems[g], owned_nnodes[g], local_nnodes[g]);
            }
            ...
            for (int le = 0; le < local_nelems[g]; le++) {
                ...
                if (h_elem_assigned_gpu[e] != g) {
                    printf("[BAD] elem %d local on GPU[%d], but assigned GPU[%d]\n", e, g,
            ...
            for (int i = 0; i < owned_nnodes[g]; i++) {
                ...
                if (n < 0 || n >= num_nodes || h_node_gpu_ind[n] != g) {
                    printf("[BAD] GPU[%d] owned node list entry %d node=%d owner=%d\n", g, i, n,
   ...
   private:
    void check_component_ids() const {
        for (int e = 0; e < num_elements; e++) {
            ...
            if (c < 0 || c >= num_components) {
                printf(
                    "[TacsComponentGPUPartitioner] invalid component id: elem %d comp %d "
                    "num_components %d\n",
                    e, c, num_components);
                exit(1);
            }
    ...
    void split_elem_connectivity() {
        ...
        int *n2e_vals = new int[n2e_ptr[num_nodes]];
        std::memset(n2e_cts, 0, num_nodes * sizeof(int));
        ...
        int *e2e_vals = new int[e2e_ptr[num_elements]];
        std::memset(e2e_cts, 0, num_elements * sizeof(int));
        ...
        int *c2c_vals = new int[c2c_ptr[num_components]];
        std::memset(c2c_ctr, 0, num_components * sizeof(int));
        ...
        for (int g = 0; g < ngpus; g++) {
            ...
            if (!debug) {
                CHECK_CUDA(cudaSetDevice(debug ? 0 : g));
                CHECK_CUDA(cudaMalloc((void **)&d_local_elem_conn[g],
                                      local_nelems[g] * nodes_per_elem * sizeof(int)));
                CHECK_CUDA(cudaMalloc((void **)&d_local_elems[g], local_nelems[g] * sizeof(int)));
            }
        }
        ...
        if (!debug) {
            for (int g = 0; g < ngpus; g++) {
                ...
                CHECK_CUDA(cudaMemcpy(d_local_elem_conn[g], h_local_elem_conn[g],
                                      local_nelems[g] * nodes_per_elem * sizeof(int),
                                      cudaMemcpyHostToDevice));
                CHECK_CUDA(cudaMemcpy(d_local_elems[g], h_local_elems[g],
                                      local_nelems[g] * sizeof(int), cudaMemcpyHostToDevice));
            }
        }
    ...
    void build_local_node_maps() {
        ...
        for (int g = 0; g < ngpus; g++) {
            ...
            for (int i = 0; i < local_nelems[g] * nodes_per_elem; i++) {
                node_set.insert(h_local_elem_conn[g][i]);
            }
    ...
    void move_maps_to_device() {
        ...
        for (int dst = 0; dst < ngpus; dst++) {
            for (int src = 0; src < ngpus; src++) {
                ...
                CHECK_CUDA(cudaSetDevice(debug ? 0 : dst));
                CHECK_CUDA(
                    cudaMalloc((void **)&d_dstred_map[idx], srcdest_nnodes[idx] * sizeof(int)));

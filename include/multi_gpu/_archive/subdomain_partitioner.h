...
class SubdomainGPUPartitioner {
    // allows taking an IEV partitioner and building a reduced IE, I or Vc partitioner for example
    // here it just uses IE as one example of reduced nodes on subdomains..
   public:
    SubdomainGPUPartitioner(int ngpus_, int num_nodes_, int *IE_nnodes_, int **IE_nodes_,
                            int **IEV_nodes_, IEVPartition *part_IEV_, bool debug_ = false)
        : ngpus(ngpus_),
          ...
          part_IEV(part_IEV_),
          debug(debug_) {
        if (debug) printf("[SDPartition] begin ngpus=%d num_nodes=%d\n", ngpus, num_nodes);

        if (debug) printf("[SDPartition] build_IE_offsets\n");
        build_IE_offsets();
        if (debug) printf("[SDPartition] num_IE_nodes=%d\n", num_IE_nodes);

        if (debug) printf("[SDPartition] build_global_to_IE_map\n");
        build_global_to_IE_map();
    ...
    void free() {
        for (int g = 0; g < ngpus; g++) {
            ...
            if (h_IEV_to_local_map && h_IEV_to_local_map[g]) delete[] h_IEV_to_local_map[g];
            if (h_local_to_IEV_map && h_local_to_IEV_map[g]) delete[] h_local_to_IEV_map[g];
            if (h_local_to_nred_map && h_local_to_nred_map[g]) delete[] h_local_to_nred_map[g];
            if (d_owned_nodes && d_owned_nodes[g]) cudaFree(d_owned_nodes[g]);
            if (d_local_nodes && d_local_nodes[g]) cudaFree(d_local_nodes[g]);
            ...
            if (d_IEV_to_local_map && d_IEV_to_local_map[g]) cudaFree(d_IEV_to_local_map[g]);
            if (d_local_to_IEV_map && d_local_to_IEV_map[g]) cudaFree(d_local_to_IEV_map[g]);
            if (d_local_to_nred_map && d_local_to_nred_map[g]) cudaFree(d_local_to_nred_map[g]);
        }
        ...
        delete[] d_local_nodes;
        delete[] h_owned_to_local_map;
        delete[] h_local_to_owned_map;
        delete[] d_owned_to_local_map;
        delete[] d_local_to_owned_map;
        delete[] h_IEV_to_local_map;
        delete[] h_local_to_IEV_map;
        delete[] d_IEV_to_local_map;
        delete[] d_local_to_IEV_map;
        delete[] h_local_to_nred_map;
        delete[] d_local_to_nred_map;
        delete[] ghost_nnodes;
   ...
   private:
    void build_IE_offsets() {
        ...
        IE_offsets[0] = 0;
        for (int g = 0; g < ngpus; g++) IE_offsets[g + 1] = IE_offsets[g] + IE_nnodes_in[g];
        num_IE_nodes = IE_offsets[ngpus];
    }
    void build_global_to_IE_map() {
        h_nred_to_IE = new int[num_nodes];
        h_node_gpu_ind = new int[num_IE_nodes];
        std::fill(h_nred_to_IE, h_nred_to_IE + num_nodes, -1);
        std::fill(h_node_gpu_ind, h_node_gpu_ind + num_IE_nodes, -1);
        for (int g = 0; g < ngpus; g++) {
            for (int i = 0; i < IE_nnodes_in[g]; i++) {
                int ie = IE_offsets[g] + i;
                int nred = IEV_nodes_in[g][i];
                h_nred_to_IE[nred] = ie;
                h_node_gpu_ind[ie] = g;
            }
        }
    }
    void build_owned_node_lists() {
        ...
        for (int g = 0; g < ngpus; g++) {
            ...
            for (int i = 0; i < owned_nnodes[g]; i++) h_owned_nodes[g][i] = IE_offsets[g] + i;
        }
    }
    void build_local_node_lists() {
        ...
        for (int g = 0; g < ngpus; g++) {
            ...
            for (int iev = 0; iev < IEV_local_nnodes; iev++) {
                int nred = h_IEV_local_nodes[iev];
                if (h_nred_to_IE[nred] >= 0) local_nnodes[g]++;
            }
            ...
            for (int iev = 0; iev < IEV_local_nnodes; iev++) {
                int nred = h_IEV_local_nodes[iev];
                int ie = h_nred_to_IE[nred];
                if (ie < 0) continue;
                h_local_nodes[g][ct] = ie;
                h_local_to_IEV_map[g][ct] = iev;
                h_local_to_nred_map[g][ct] = nred;
                h_IEV_to_local_map[g][iev] = ct;
                ct++;
            }
        }
    }
    ...
    void build_ghost_node_maps() {
        ...
        for (int g = 0; g < ngpus; g++)
            for (int i = 0; i < owned_nnodes[g]; i++) owned_pos[h_owned_nodes[g][i]] = i;
   ...
   public:
    int ngpus = 0, num_nodes = 0, num_IE_nodes = 0;

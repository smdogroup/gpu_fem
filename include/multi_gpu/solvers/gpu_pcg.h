...
class GPU_PCG {
   public:
    ...
    void print_progress_bar(int iter, int max_iter, T resid_norm, T init_resid_norm, T conv_tol,
        ...
        // We keep cursor parked on a blank spacer line beneath the progress bar.
        // Each update moves up to the bar line, rewrites it, then returns to spacer.
        std::printf("\033[1A");   // spacer line -> progress line
        std::printf("\r\033[K");  // clear progress line
        ...
        for (int j = 0; j < max_iter; j++, total_iter++) {
            ...
            if (std::abs(resid_norm) < conv_tol) {
                ...
                if (can_print && use_progress_bar) {
                    ...
                    // Final newline plus temp blank line below progress bar
                    std::printf("\n");
                }

...
const char *TacsMeshLoaderElementTypes[] = {"CBAR",  "CQUADR",  "CQUAD4", "CQUAD8", "CQUAD9",
...
class TACSMeshLoader {
   public:
    ...
    //     TACSAssembler::NATURAL_ORDER, TACSAssembler::MatrixOrderingType
    //     mat_type =
    ...
    void getConnectivity(int *_num_nodes, int *_num_elements, const int **_elem_node_ptr,
                         const int **_elem_node_conn, const int **_elem_component, const T **_Xpts);
    ...
    void getAssemblerCreatorData(const int vars_per_node, int &my_num_nodes, int &my_num_elements,
        ...
        my_num_components = num_components;
        ...
        // deep copy
        my_elem_components = new int[num_elements];
        for (int ielem = 0; ielem < num_elements; ielem++) {
            my_elem_components[ielem] = elem_component[ielem];
        }
...
// */
// const char *TACSMeshLoader<T>::getElementDescript(int comp_num) {
...
*/
template <typename T>
int TACSMeshLoader<T>::scanBDFFile(const char *file_name) {
    // int rank;
    // MPI_Comm_rank(comm, &rank);
    int fail = 0;
    int rank = 0;  // we are on host but no MPI in GPU version for now
            ...
            if (line[0] != '$') {
                if (in_bulk &&
                    ...
                    if (index >= 0) {
                        ...
                        // Check if the number of nodes is within the prescribed
                        // limits
                        if (num_conn < TacsMeshLoaderElementLimits[index][0]) {
                            fprintf(stderr,
                                    "TACSMeshLoader: Number of nodes for "
                                    "element %s "
                                    "not within limits\n",
        ...
        while (buffer_loc < buffer_len) {
            ...
            if (line[0] != '$') {  // A comment line
                if (in_bulk &&
                    ...
                    // This is a variable-length format. Read in grid points
                    // until zero is reached. This is a fixed-width format SPC*
                    // SID  G1  C  D

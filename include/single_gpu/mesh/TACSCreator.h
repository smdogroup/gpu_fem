...
#pragma once
#include "legacy/TACSObject.h"

// orderings of the matrix, etc.
enum OrderingType {
...
enum MatrixOrderingType { ADDITIVE_SCHWARZ, APPROXIMATE_SCHUR, DIRECT_SCHUR, GAUSS_SEIDEL };
...
class TACSCreator : public TACSObject {
   public:
    ...
    // ----------------------------------------
    void setGlobalConnectivity(int _num_nodes, int _num_elements, const int *_elem_node_ptr,
                               const int *_elem_node_conn, const int *_elem_id_nums);

    // Set the boundary conditions
    // ---------------------------
    void setBoundaryConditions(int _num_bcs, const int *_bc_nodes, const int *_bc_ptr = NULL,
                               const int *_bc_vars = NULL, const TacsScalar *_bc_vals = NULL);

    // Set the dependent node connectivity and weights
    // -----------------------------------------------
    void setDependentNodes(int num_dep_nodes, const int *_dep_node_ptr, const int *_dep_node_conn,
    ...
    // -------------------------------
    void setReorderingType(OrderingType _order_type, MatrixOrderingType _mat_type);
    ...
    void getAssemblerCreatorData(const int vars_per_node, int &my_num_nodes, int &my_num_elements,
                                 int &my_num_bcs, int &my_num_components, int *&my_elem_conn,
                                 int *&my_bcs, int *&my_elem_components, TacsScalar *&my_xpts);
...
#include "legacy/TACSUtilities.h"
#include "legacy/tacsmetis.h"
...
void TACSCreator::setGlobalConnectivity(int _num_nodes, int _num_elements,
    ...
    memcpy(elem_node_conn, _elem_node_conn, elem_node_ptr[num_elements] * sizeof(int));
...
void TACSCreator::setDependentNodes(int num_dep_nodes, const int *_dep_node_ptr,
                                    const int *_dep_node_conn, const double *_dep_node_weights) {
...
*/
void TACSCreator::setBoundaryConditions(int _num_bcs, const int *_bc_nodes, const int *_bc_ptr,
                                        const int *_bc_vars, const TacsScalar *_bc_vals) {
...
/*
  Set the type of ordering to use
*/
void TACSCreator::setReorderingType(OrderingType _order_type, MatrixOrderingType _mat_type) {
    use_reordering = 1;
...
*/
void TACSCreator::getNumOwnedNodes(int **_owned_nodes) { *_owned_nodes = owned_nodes; }

/*
  Get the number of elements owned by each processor
*/
void TACSCreator::getNumOwnedElements(int **_owned_elements) { *_owned_elements = owned_elements; }
...
void TACSCreator::getAssemblerCreatorData(const int vars_per_node, int &my_num_nodes,
                                          int &my_num_elements, int &my_num_bcs,
                                          int &my_num_components, int *&my_elem_conn, int *&my_bcs,
                                          int *&my_elem_components, TacsScalar *&my_xpts) {
    int size, rank;
    MPI_Comm_size(comm, &size);
    MPI_Comm_rank(comm, &rank);
    ...
    num_owned_nodes = 0;
    num_owned_elements = 0;
    MPI_Scatter(owned_nodes, 1, MPI_INT, &num_owned_nodes, 1, MPI_INT, root_rank, comm);
    MPI_Scatter(owned_elements, 1, MPI_INT, &num_owned_elements, 1, MPI_INT, root_rank, comm);
    ...
    if (rank == root_rank) {
        ...
        for (int k = 0; k < size; k++) {
            ...
            if (local_dep_node_size > 0) {
                ...
                for (int j = 0; j < local_dep_node_size; j++) {
                    ...
                    for (int i = dep_node_ptr[dep_node]; i < dep_node_ptr[dep_node + 1]; i++) {
            ...
            if (k == root_rank) {
                ...
                if (num_local_dep_nodes > 0) {
                    ...
                    memcpy(local_dep_node_ptr, dep_ptr, (num_local_dep_nodes + 1) * sizeof(int));
                ...
                memcpy(local_elem_node_ptr, elem_ptr, (num_owned_elements + 1) * sizeof(int));
        ...
        Xpts_local = new TacsScalar[3 * num_owned_nodes];
        MPI_Recv(Xpts_local, 3 * num_owned_nodes, TACS_MPI_TYPE, root_rank, 4, comm, &status);

        // Receive the dependent node data if there are any dependent
        // nodes on this processor
        if (num_local_dep_nodes > 0) {
            local_dep_node_ptr = new int[num_local_dep_nodes + 1];
            MPI_Recv(local_dep_node_ptr, num_local_dep_nodes + 1, MPI_INT, root_rank, 5, comm,
                     &status);
            ...
            local_dep_node_conn = new int[conn_len];
            local_dep_node_weights = new double[conn_len];
            MPI_Recv(local_dep_node_conn, conn_len, MPI_INT, root_rank, 6, comm, &status);
            MPI_Recv(local_dep_node_weights, conn_len, MPI_DOUBLE, root_rank, 7, comm, &status);
        }

        // Receive the element data
        MPI_Recv(local_elem_id_nums, num_owned_elements, MPI_INT, root_rank, 8, comm, &status);
        MPI_Recv(local_elem_node_ptr, num_owned_elements + 1, MPI_INT, root_rank, 9, comm, &status);

        int con_size = local_elem_node_ptr[num_owned_elements];
        local_elem_node_conn = new int[con_size];
        MPI_Recv(local_elem_node_conn, con_size, MPI_INT, root_rank, 10, comm, &status);
    }
    ...
    // copy data to element assembler here
    // -----------------------------------
    ...
    for (int i = 0; i < con_size; i++) {
        my_elem_conn[i] = local_elem_node_conn[i];
    }
    ...
    // deep copy
    my_xpts = new TacsScalar[3 * num_owned_nodes];
    for (int j = 0; j < 3 * num_owned_nodes; j++) {
    ...
    // write outputs here
    // ------------------
    // recall outputs to assembler creator data
    // const int vars_per_node, int &my_num_nodes, int &my_num_elements,
    //  int &my_num_bcs, int &my_num_components, int *&my_elem_conn,
    //  int *&my_bcs, int *&my_elem_components, T *&my_xpts
...
void TACSCreator::partitionMesh(int split_size, const int *part) {
    ...
    if (!partition) {
        ...
        if (split_size > 1) {
            ...
            if (split_size < 8) {
                METIS_PartGraphRecursive(&num_elements, &ncon, elem_ptr, elem_conn, NULL, NULL,
                                         NULL, &split_size, NULL, NULL, options, &objval,
                                         partition);
            } else {
                METIS_PartGraphKway(&num_elements, &ncon, elem_ptr, elem_conn, NULL, NULL, NULL,
                                    &split_size, NULL, NULL, options, &objval, partition);
            }
        } else {

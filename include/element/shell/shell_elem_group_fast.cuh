...
__GLOBAL__ void fast_drill_strain_add_residual(const int32_t num_elements, const Vec<int32_t> geo_conn,
...
#pragma unroll
    ...
    // apparently memset uses vectorized instructions.. and doesn't create extra registers for setting this here..
    ...
    if constexpr (fast_drill_strain) {
        ElemGroup::template _add_drill_strain_quadpt_residual_fast<Data>(active_thread, iquad, shared_xpts,
            shared_vars, shared_data, shared_vars, shared_vars, local_res);
    }
    ...
    if constexpr (include_drill_strain) {
        ...
        ElemGroup::template _add_drill_strain_quadpt_residual_fast<Data>(active_thread, iquad, shared_xpts,
            shared_vars, shared_data, block_Tmat[local_elem], block_XdinvT[local_elem], local_res);
    }
    ...
    // will go from 3.4e-4 to 3.2e-4 with atomicAdds (what? weird.., so graph coloring may not help..)

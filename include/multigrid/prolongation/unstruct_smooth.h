...
class UnstructuredSmoothProlongation {
   public:
    ...
    /* 
    difference in this and the regular unstructured prolongation, is the sparsity pattern is A*P0 insetad of just P0 (includes one fillin step)
    this way it will smooth better */
    ...
    void construct_nz_pattern() {
        ...
        const include_restrict = false; // don't get PT matrix also
    ...
    void apply_kmat_fillin() {
        ...
        cudaMemfree(d_P_rows);
        cudaMemfree(d_P_cols);
    ...
    void assemble_matrices() {
        ...
        k_prolong_mat_assembly<T, Basis, is_bsr><<<grid, block>>>(
            ...
            d_fine_iperm, d_P_rowp, d_P_cols, block_dim, d_P_vals);
    }
    ...
    void restrict_vec(DeviceVec<T> fine_vec_in, DeviceVec<T> coarse_vec_out) {
        ...
        k_bsrmv_transpose<T><<<grid0, block0>>>(P_nnzb, block_dim, d_P_rows, d_P_cols, d_P_vals, fine_vec_in.getPtr(), coarse_vec_out.getPtr());

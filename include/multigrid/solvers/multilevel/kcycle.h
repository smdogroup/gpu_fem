...
public:
    ...
    void init_outer_solver(cublasHandle_t &cublasHandle, cusparseHandle_t &cusparseHandle, 
        int n_smooth, int n_cycles, int n_krylov, T omega = 1.0, T atol = 1e-6, T rtol = 1e-6, 
        int print_freq = 1, bool print = true, bool double_smooth = false) {
    ...
    void init_solvers(cublasHandle_t &cublasHandle, cusparseHandle_t &cusparseHandle, 
        ...
        for (int ilevel = nlevels - 1; ilevel >= 0; ilevel--) {
            if (ilevel == nlevels-1) {
                ...
                coarse_solver = new CoarseSolver(cublasHandle, cusparseHandle, grids[ilevel].assembler, grids[ilevel].Kmat);
                ...
                if (just_outer_krylov) {
                    ...
                    BaseSolver *subspace_solver = new SubspaceSolver(cublasHandle, cusparseHandle, &grids[ilevel], &grids[ilevel+1], solvers[isolver-1], copy_options);
                    ...
                    if (ilevel == 0) {
                        ...
                        BaseSolver *krylov_solver = new KrylovSolver(cublasHandle, cusparseHandle, &grids[ilevel], subspace_solver, krylov_options, ilevel);

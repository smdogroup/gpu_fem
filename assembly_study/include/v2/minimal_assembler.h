...
// input is either a device array when USE_GPU or a host array if not USE_GPU
    ...
    // ------------------------
    ElementAssemblerV2(int32_t num_geo_nodes_, int32_t num_vars_nodes_, int32_t num_elements_,
    ...
    static ElementAssemblerV2 createFromBDF(TACSMeshLoader<T> &mesh_loader, Data single_data);
   ...
   private:
    ...
    Vec<Data> physData;
    BsrData bsr_data;
};  // end of ElementAssemblerV2 class declaration

template <typename T, typename ElemGroup, template <typename> class Vec,
          template <typename> class Mat>
ElementAssemblerV2<T, ElemGroup, Vec, Mat>::ElementAssemblerV2(
...
template <typename T, typename ElemGroup, template <typename> class Vec,
          template <typename> class Mat>
ElementAssemblerV2<T, ElemGroup, Vec, Mat>
ElementAssemblerV2<T, ElemGroup, Vec, Mat>::createFromBDF(TACSMeshLoader<T> &mesh_loader,
                                                          Data single_data) {
    ...
    return ElementAssemblerV2(num_nodes, num_nodes, num_elements, elem_conn_vec, elem_conn_vec,
                              xpts_vec, bcs_vec, physData_vec, num_components, elem_components_vec);
}

template <typename T, typename ElemGroup, template <typename> class Vec,
          template <typename> class Mat>
void ElementAssemblerV2<T, ElemGroup, Vec, Mat>::moveBsrDataToDevice() {
#ifdef USE_GPU
...
template <typename T, typename ElemGroup, template <typename> class Vec,
          template <typename> class Mat>
__HOST__ void ElementAssemblerV2<T, ElemGroup, Vec, Mat>::apply_bcs(Vec<T> &vec, bool can_print) {
    if (can_print) {
...
template <typename T, typename ElemGroup, template <typename> class Vec,
          template <typename> class Mat>
void ElementAssemblerV2<T, ElemGroup, Vec, Mat>::apply_bcs(Mat &mat, bool can_print) {
    if (can_print) {
...
template <typename T, typename ElemGroup, template <typename> class Vec,
          template <typename> class Mat>
HostVec<T> ElementAssemblerV2<T, ElemGroup, Vec, Mat>::createVarsHostVec(T *data, bool randomize) {
    HostVec<T> h_vec;
    if (data == nullptr) {
...
template <typename T, typename ElemGroup, template <typename> class Vec,
          template <typename> class Mat>
DeviceVec<T> ElementAssemblerV2<T, ElemGroup, Vec, Mat>::createVarsVec(T *data, bool randomize,
                                                                       bool can_print) {
    if (can_print) {
...
template <typename T, typename ElemGroup, template <typename> class Vec,
          template <typename> class Mat>
HostVec<T> ElementAssemblerV2<T, ElemGroup, Vec, Mat>::createVarsVec(T *data, bool randomize) {
...
template <typename T, typename ElemGroup, template <typename> class Vec,
          template <typename> class Mat>
void ElementAssemblerV2<T, ElemGroup, Vec, Mat>::set_variables(Vec<T> &newVars) {
...
template <typename T, typename ElemGroup, template <typename> class Vec,
          template <typename> class Mat_>
void ElementAssemblerV2<T, ElemGroup, Vec, Mat_>::add_jacobian(

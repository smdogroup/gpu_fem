...
A2D_FUNCTION void IsotropicShellStressCore(const T& E, const T& nu, const T& thick,
                                           const T& tOffset, T strain[], T stress[]) {
    ...
    A2D::SymMatVecCoreScale3x3<T, true>(thick * thick * thick * tOffset * tOffset, C, &strain[3],
                                        &stress[3]);
...
template <typename T, class Data>
A2D_FUNCTION void IsotropicShellStressReverseCore(const T& E, const T& nu, const T& thick,
                                                  const T& tOffset, T stressb[], T strainb[]) {
    // stress and strain are length 9 vectors in pointer form now
    // need to compute A,B,D,As matrices etc
    T C[6];
    Data::evalTangentStiffness2D(E, nu, C);
    ...
    A2D::SymMatVecCoreScale3x3<T, true>(thick * thick * thick * tOffset * tOffset, C, &stressb[3],
                                        &strainb[3]);
...
template <typename T, class Data>
A2D_FUNCTION void IsotropicShellStress(const T& E, const T& nu, const T& thick, const T& tOffset,
...
class IsotropicShellStressExpr {
   public:
    ...
    static_assert((nstrain == nstress), "Shell Stress Expression does not have right size..");
    ...
    static_assert(get_diff_order<strainType>::order == order, "ADorder does not match");
    ...
    A2D_FUNCTION void eval() {
        IsotropicShellStressCore<T, Data>(E, nu, thick, tOffset, get_data(strain),
                                          get_data(stress));
    }
    ...
    A2D_FUNCTION void forward() {
        ...
        IsotropicShellStressCore<T, Data>(E, nu, thick, tOffset, GetSeed<seed>::get_data(strain),
                                          GetSeed<seed>::get_data(stress));
    }

    A2D_FUNCTION void reverse() {
        constexpr ADseed seed = ADseed::b;
        IsotropicShellStressReverseCore<T, Data>(E, nu, thick, tOffset,
                                                 GetSeed<seed>::get_data(stress),
                                                 GetSeed<seed>::get_data(strain));
    }
...
// fix this later
template <typename T, class Data>
A2D_FUNCTION auto IsotropicShellStress(const T& E, const T& nu, const T& thick, const T& tOffset,
                                       A2D::ADObj<A2D::Vec<T, 9>>& strain,
                                       A2D::ADObj<A2D::Vec<T, 9>>& stress) {
    return IsotropicShellStressExpr<Data, A2D::ADObj<A2D::Vec<T, 9>>, A2D::ADObj<A2D::Vec<T, 9>>>(
        E, nu, thick, tOffset, strain, stress);
}

template <typename T, class Data>
A2D_FUNCTION auto IsotropicShellStress(const T& E, const T& nu, const T& thick, const T& tOffset,
                                       A2D::A2DObj<A2D::Vec<T, 9>>& strain,
                                       A2D::A2DObj<A2D::Vec<T, 9>>& stress) {
    return IsotropicShellStressExpr<Data, A2D::A2DObj<A2D::Vec<T, 9>>, A2D::A2DObj<A2D::Vec<T, 9>>>(
        E, nu, thick, tOffset, strain, stress);
}
...
//   return IsotropicShellStressExpr<Data, strainType, stressType>(E, nu, thick, tOffset, strain,
//   stress);
// }

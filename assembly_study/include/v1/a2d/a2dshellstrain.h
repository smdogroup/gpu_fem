...
template <typename T>
A2D_FUNCTION void LinearShellStrainForwardCore(const T u0x[], const T u1x[], const T e0ty[],
                                               const T et[], T e[]) {
    // Evaluate the in-plane strains from the tying strain expressions
    e[0] = e0ty[0];        // e11
    e[1] = e0ty[3];        // e22
...
template <typename T>
A2D_FUNCTION void LinearShellStrainReverseCore(const T eb[], T u0xb[], T u1xb[], T e0tyb[],
                                               T etb[]) {
    // Evaluate the in-plane strains from the tying strain expressions
    e0tyb[0] += eb[0];        // e1
    e0tyb[3] += eb[1];        // e22
...
template <typename T>
A2D_FUNCTION void NonlinearShellStrainCore(const T u0x[], const T u1x[], const T e0ty[],
                                           const T et[], T e[]) {
    // Evaluate the in-plane strains from the tying strain expressions
    e[0] = e0ty[0];        // e11
    e[1] = e0ty[3];        // e22
...
template <typename T>
A2D_FUNCTION void NonlinearShellStrainForwardCore(const T u0x[], const T u1x[], const T e0ty[],
                                                  const T et[], const T u0xF[], const T u1xF[],
                                                  const T e0tyF[], const T etF[], T eF[]) {
    // Evaluate the in-plane strains from the tying strain expressions
    eF[0] = e0tyF[0];        // e11
    eF[1] = e0tyF[3];        // e22
    ...
    // Compute the bending strain (here's where nonlinearity comes in)
    eF[3] = u1xF[0] + (u0xF[0] * u1x[0] + u0x[0] * u1xF[0] + +u0xF[3] * u1x[3] + u0x[3] * u1xF[3] +
                       u0xF[6] * u1x[6] + u0x[6] * u1xF[6]);  // k11
    eF[4] = u1xF[4] + (u0xF[1] * u1x[1] + u0x[1] * u1xF[1] + u0xF[4] * u1x[4] + u0x[4] * u1xF[4] +
                       u0xF[7] * u1x[7] + u0x[7] * u1xF[7]);  // k22
    eF[5] = u1xF[1] + u1xF[3] +
            (u0xF[0] * u1x[1] + u0x[0] * u1xF[1] + u0xF[3] * u1x[4] + u0x[3] * u1xF[4] +
...
template <typename T>
A2D_FUNCTION void NonlinearShellStrainReverseCore(const T eb[], const T u0x[], const T u1x[],
                                                  const T e0ty[], const T et[], T u0xb[], T u1xb[],
                                                  T e0tyb[], T etb[]) {
...
template <typename T>
A2D_FUNCTION void NonlinearShellStrainHessianReverseCore(const T eh[], const T eb[], const T u0x[],
                                                         const T u1x[], const T e0ty[],
                                                         const T et[], const T u0xp[],
                                                         const T u1xp[], const T e0typ[],
                                                         const T etp[], T u0xh[], T u1xh[],
                                                         T e0tyh[], T eth[]) {
